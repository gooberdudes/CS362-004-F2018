...
int playAdventurer(int drawntreasure, struct gameState *state, int *temphand){
  int currentPlayer = whoseTurn(state);
  ...
  drawCard(currentPlayer, state);g
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      playAdventurer(drawntreasure, state, temphand);
    ...
    case council_room:
      ...
      for (i = 0; i < 4; i++)
	{
	  drawCard(currentPlayer, state, temphand);
	}
